...
| `dependency-check-java` | OWASP dependency-check over `kelta-platform`, `failBuildOnCVSS=7`, using the existing `dependency-check-suppressions.xml`. **Gated on the `NVD_API_KEY` secret** — unkeyed NVD access has been rate-limited since 2023 to the point of timing out. Without the secret it emits a `::warning` saying the Java tree is unscanned and skips; `quality-gate` permits skipped jobs, so it becomes a real gate the moment the key is added (Settings → Secrets → Actions → `NVD_API_KEY`). Invoked explicitly rather than phase-bound, so the NVD download does not attach to every `mvn verify`. |
