...
- **kelta.io apex/www/downloads SERVFAIL'd from inside the cluster (2026-09-21, homelab-argo PR #312, commit `fb0066a`, merged 2026-09-21T05:30:48Z)** — the e2e "Wait for public DNS to serve the marketing site" step runs on the in-cluster `k8s-runner`, and every `workflow_dispatch` deploy ended `failure` on it with `HTTP 000` after a successful deploy. Not a public-DNS problem (the apex A record had already been repointed at `71.218.137.106`): homelab-argo's `coredns/configmap.yaml` `kelta.io:53` block was `hosts { … fallthrough }` with **no next plugin**, so CoreDNS answered every kelta.io name outside the hosts list with SERVFAIL and logged `plugin/hosts: no next plugin found`. Fixed in homelab-argo (not this repo) by adding `kelta.io www.kelta.io downloads.kelta.io` to the split-horizon hosts line (same Traefik VIP `192.168.0.22` as api/app/auth/mcp) and a `forward . 1.1.1.1 8.8.8.8` after `hosts`. Live-verified from an in-cluster pod (PLT-303, 2026-09-21, resolver `10.152.183.10`/CoreDNS via `/etc/resolv.conf`): `getent hosts kelta.io`/`www.kelta.io`/`downloads.kelta.io` all → `192.168.0.22`; `curl -sk https://kelta.io/`, `https://www.kelta.io/` and `https://downloads.kelta.io/` all → `HTTP 200`. Lesson: a CoreDNS `hosts` block with `fallthrough` must be followed by a `forward`, or the "fall through" is a SERVFAIL.
