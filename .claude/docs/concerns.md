...
- **kelta.io apex/www/downloads SERVFAIL'd from inside the cluster (2026-09-21, homelab-argo PR #312, commit `fb0066a`, merged 2026-09-21T05:30:48Z)** — the e2e "Wait for public DNS to serve the marketing site" step runs on the in-cluster `k8s-runner`, and every `workflow_dispatch` deploy ended `failure` on it with `HTTP 000` after a successful deploy. Not a public-DNS problem (the apex A record had already been repointed at `71.218.137.106`): homelab-argo's `coredns/configmap.yaml` `kelta.io:53` block was `hosts { … fallthrough }` with **no next plugin**, so CoreDNS answered every kelta.io name outside the hosts list with SERVFAIL and logged `plugin/hosts: no next plugin found`. Fixed in homelab-argo by adding `kelta.io www.kelta.io downloads.kelta.io` to the split-horizon hosts line (same Traefik VIP `192.168.0.22` as api/app/auth/mcp) and a `forward . 1.1.1.1 8.8.8.8` after `hosts`. Re-verified live from an in-cluster pod (PLT-307, 2026-09-22): `getent hosts` resolved `kelta.io`, `www.kelta.io` and `downloads.kelta.io` all to `192.168.0.22` via CoreDNS, and `curl -k` against `https://kelta.io/`, `https://www.kelta.io/` and `https://downloads.kelta.io/` returned `HTTP 200`. Independently re-verified again (PLT-309, 2026-09-24T12:21:11Z, in-cluster pod `rzware-worker-plt-309-a1-r6hcz`, `/etc/resolv.conf` nameserver `10.152.183.10`): `getent hosts` still resolves all three names to `192.168.0.22`, and `curl -sk -o /dev/null -w "%{http_code}"` against all three URLs still returns `200`. The fix continues to hold. Lesson: a CoreDNS `hosts` block with `fallthrough` must be followed by a `forward`, or the "fall through" is a SERVFAIL.
- **New collection 404s at the gateway after create — concurrent route-cache refreshes lose the newer snapshot (2026-09-21, #1579 CI)** — Spring Cloud Gateway's `CachingRouteLocator` handles `RefreshRoutesEvent` as fetch-then-store with only the store synchronized. A collection create publishes two NATS events — `kelta.config.collection.changed` (`ConfigEventListener` registers the route, then refreshes) and `kelta.record.changed` for `collections` (`SystemCollectionRouteListener` refreshes) — on two consumer threads, so the record-changed refresh could snapshot the registry *before* the route landed and store that stale snapshot *after* the fresh one. Result: `404 No static resource api/<name>` for the new collection until the next config event; six harness scenarios timed out on `/api/customers`. All four refresh sites now go through `route/RouteRefresher.refresh()`, whose `synchronized` publish serializes the whole fetch-and-store (the multicaster is synchronous and the locator in-memory, so the rebuild completes inside `publishEvent`). Never publish `RefreshRoutesEvent` directly. `RouteRefresherTest` pins the serialization; `ConfigEventListenerTest` pins register-then-refresh order.
