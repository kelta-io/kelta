...
**Direct DB assertions**: `ScenarioBase.openDbConnection()` opens a JDBC connection from the test JVM (via `KeltaStack.dbJdbcUrl()`, which maps the local container port / passes the CI URL through) for asserting table state the API doesn't expose — e.g. `flow_pending_resume` in `FlowWaitResumeScenarioTest`. That user is the image's bootstrap superuser and **bypasses RLS even on FORCE'd tables**, which is what makes it useful for planting and inspecting fixtures. **The services themselves connect as a separate `NOBYPASSRLS` role** that `KeltaStack.provisionApplicationRole()` creates per run (`kelta_app` locally, `app_<run schema>` on the CI pool, dropped by `scripts/ci/release-db.sh`), so every scenario exercises the real policies. That role also runs **Flyway** — the only non-superuser run of the migrations besides `RowLevelSecurityIntegrationTest`, which migrates the same way — so it is made the owner of the target schema and gets its extensions installed for it; see `concerns.md` → row-level security for why both are load-bearing. For an RLS assertion use `ScenarioBase.openAppDbConnection()` — that same role — and bind a tenant with `SET LOCAL app.current_tenant_id` inside a transaction (`TenantIsolationScenarioTest.databaseDeniesCrossTenantReads`); `openDbConnection(user, password)` is still there for a purpose-built probe role, as `RecordShareWideningScenarioTest` uses. Also note V77's RLS loop targets only the `public` schema **by design**: `public` is the shared multi-tenant schema, while any non-public schema (including the CI pool's throwaway `ci_*` schemas) is tenant-/run-specific and needs no RLS gating. Practical consequence on CI: only migrations that `ALTER TABLE` directly (e.g. V150 `record_share`) carry RLS policies there, so scope harness RLS assertions to those tables, not V77-listed ones.
